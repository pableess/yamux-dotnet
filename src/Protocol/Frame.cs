...
internal readonly record struct Frame(FrameHeader Header, ReadOnlyMemory<byte> Payload) : IDisposable
{
    public IDisposable? BufferOwner { get; init; }
    ...
    public static Frame CreateDataFrame(uint streamId, Flags flags, ReadOnlyMemory<byte> payload, IDisposable? bufferOwner = null)
    {
        var header = new FrameHeader(ProtocolVersion.Initial, FrameType.Data, flags, streamId, (uint)payload.Length);
        return new Frame(header, payload) { BufferOwner = bufferOwner };
    }
